...
const AdminDashboard = () => {
  ...
  const totalWeeklyKwh = dailyPowerData.reduce((sum, d) => sum + d.kwh, 0);
                  ...
                  {monthlyPowerData.length > 0 && (
                    <>
                      <div className="p-3 rounded-lg border bg-muted/30 col-span-2">
                        <span className="data-label">Latest Month ({monthlyPowerData[monthlyPowerData.length - 1].month})</span>
                        <div className="font-bold font-mono text-foreground">
                          {monthlyPowerData[monthlyPowerData.length - 1].totalKwh.toFixed(1)} kWh
                        </div>
                        <div className="text-xs text-muted-foreground">
                          ₱{(monthlyPowerData[monthlyPowerData.length - 1].totalKwh * vecoRate).toFixed(2)}
                        </div>
                      </div>
                    </>
                  )}
